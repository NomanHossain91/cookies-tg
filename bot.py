...
async def ig_login(username: str, password: str, twofa_code: str = None):
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        # 2FA required — check both keys Instagram might send
        if data.get("two_factor_required") or data.get("two_factor_info"):
            ...
            # verificationMethod: "1" = TOTP app, "2" = SMS
            method = "1"
            ...
            if not r2.is_success:
                return {"ok": False, "error": f"2FA HTTP {r2.status_code}: {r2.text[:200]}"}
            data = r2.json()
            if not data.get("authenticated"):
                msg = data.get("message") or data.get("errors") or "2FA code ভুল"
                return {"ok": False, "error": f"❌ 2FA failed: {msg}"}
