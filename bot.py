...
async def ig_login(username: str, password: str, twofa_code: str = None):
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        if not csrf:
            return {"ok": False, "error": "Could not obtain CSRF token."}
        ...
        if r.status_code == 429:
            return {"ok": False, "error": "Rate limited — please wait a few minutes."}
        if not r.is_success:
        ...
        if data.get("checkpoint_url"):
            ...
            return {"ok": False, "error": "⚠️ Instagram checkpoint required — please log in manually in browser first."}
...
async def login_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📝 Enter your Instagram *username*:", parse_mode="Markdown")
    return USERNAME
...
async def got_twofa(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text) > 10 and text.replace(" ","").isalnum() and not text.isdigit():
        try:
            ...
            await update.message.reply_text(f"🔑 Code generated from secret key: `{code}`", parse_mode="Markdown")
        except Exception as e:
            await update.message.reply_text(f"❌ Failed to generate code from key: {e}")
            return TWOFA
    else:
