...
async def ig_login(username: str, password: str, twofa_code: str = None):
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        if r.status_code == 429:
            return {"ok": False, "error": "Rate limited — কিছুক্ষণ পর try করো।"}
        if not r.is_success:
        ...
        if data.get("checkpoint_url"):
            ...
            return {"ok": False, "error": "⚠️ Instagram checkpoint চাইছ — আগে browser এ manually login করে verify করো।"}
...
async def cancel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data.clear()
    await update.message.reply_text("❌ বাতিল করা হয়েছ।")
    return ConversationHandler.END
